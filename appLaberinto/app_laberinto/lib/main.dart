...
class _ControlPantallaState extends State<ControlPantalla> {
  ...
  List<int> matrizMapa = List.filled(144, 0); // 12x12 = 144
  ...
  void _procesarDatosEntrantes(Uint8List data) {
    ...
    // Procesamiento lineal hasta encontrar el salto de línea (\n)
    if (_bufferRX.contains('\n')) {
      ...
      // Mantiene los fragmentos incompletos en el buffer
      _bufferRX = lineas.last; 
    }
  }

  void _parsearProtocoloMapa(String trama) {
    // Estructura: "MAPA:X,Y,Orientacion:celda1,celda2,...,celda144"
    try {
      ...
      debugPrint("Error al parsear matriz: $e");
    }
  }

  void _enviarComando(String comando) {
    if (_conectado && _conexion != null) {
      // El ESP32 espera el terminador '#' en parsearTrama()
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Column(
        children: [
          // Sección Superior: Gestión Bluetooth
          Padding(
          ...
          // Sección Central: Renderizado del Lienzo
          Expanded(
            child: Center(
              child: AspectRatio(
                aspectRatio: 1.0,
          ...
          // Sección Inferior: Controles y Trama Personalizada
          Padding(
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                const SizedBox(width: 8),
                ElevatedButton(
                  onPressed: () => _enviarComando("0,50,0#"), // Gatilla el volcado
...
// ============================================================================
// MOTOR DE RENDERIZADO TOPOLÓGICO
// ============================================================================
...
class RenderizadorLaberinto extends CustomPainter {
  ...
  void _dibujarRobot(Canvas canvas, double px, double py, double w, double h, Paint paint) {
    ...
    // Dibuja un indicador de dirección basado en la orientación numérica
    final Paint paintVector = Paint()
      ..color = Colors.white
      ..strokeWidth = 2.0;

    Offset vectorEnd = Offset(cx, cy);
    if (dir == 1) vectorEnd = Offset(cx, cy - radio); // Norte
    if (dir == 2) vectorEnd = Offset(cx, cy + radio); // Sur
    if (dir == 3) vectorEnd = Offset(cx + radio, cy); // Este
    if (dir == 4) vectorEnd = Offset(cx - radio, cy); // Oeste
  ...
  bool shouldRepaint(covariant RenderizadorLaberinto oldDelegate) {
    return true; // Fuerza redibujo en cada actualización de estado
  }
}